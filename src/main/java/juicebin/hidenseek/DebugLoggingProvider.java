...
public class DebugLoggingProvider {
    ...
    public final static String PACKAGE_NAME = Bukkit.getServer().getClass().getPackage().getName();
    public final static String API_VERSION = PACKAGE_NAME.substring(PACKAGE_NAME.lastIndexOf(".") + 1, PACKAGE_NAME.length()).substring(1);
    public final static String CB_PACKAGE = "org.bukkit.craftbukkit.";
    public final static String OBC_PACKAGE = CB_PACKAGE + "v" + API_VERSION + ".";
    public final static String NMS_PACKAGE = "net.minecraft.server.v" + API_VERSION + ".";
    ...
    public void enableDebugLogging(Level defaultLevel, boolean showThread) {
        ...
        if (System.getProperty("enableDebugLogging") != null) {
            Bukkit.getLogger().warning("***Debug logging provider was already initialized somewhere else and should not be initialized again.");
            return;
        }
        ...
        global.addHandler(new Handler() {
            ...
            private Thread getThreadById(long id) {
                if (threadIds.containsKey(id)) return threadIds.get(id);
            ...
            private Plugin getPluginByLoggerName(String loggerName) {
                if (pluginLoggers.containsKey(loggerName)) return pluginLoggers.get(loggerName);
            ...
            private Level getLogLevel(String loggerName) {
                ...
                if (logger.getLevel() != null) return logger.getLevel();
                Plugin plugin = getPluginByLoggerName(loggerName);
                if (plugin != null) return plugin.getLogger().getLevel();
                return defaultLevel;
            }
            ...
            public void publish(LogRecord record) {
                ...
                String messageWithThread = new StringBuilder("[").append(date).append("] [").append(getThreadById(record.getThreadID()).getName()).append("/").append(recordLevel).append("]: ").append(rawMessage).append("\n").toString();
                String messageWithoutThread = new StringBuilder("[").append(date).append(" ").append(recordLevel).append("]: ").append(rawMessage).append("\n").toString();
    ...
    public String fromJulLevel(Level level) {
        ...
        if (level == Level.FINE || level == Level.FINER || level == Level.FINEST) return "TRACE";
        return "OFF";
    }
    ...
    public static class ConsoleReaderWrapper {
        ...
        public CursorBufferWrapper getCursorBuffer() {
            if (cursorBuffers.containsKey(this)) return cursorBuffers.get(this);
